...
df_outfit_meta = pd.read_csv(
    osp.join(data_dir, "important", "outfit_meta.csv"), index_col=None
)
...
outfit_descriptions = io.load_json(
    osp.join(data_dir, "json", "theme_aware_dataset_descriptions.json"), verbose=True
)
...
# %% Display outlier outfit with descriptions
n_outfits = len(outfit_ids)
n_sample = 8
