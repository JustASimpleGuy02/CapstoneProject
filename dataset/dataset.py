...
class PolyvoreDataset(Dataset):
    def __init__(
        self, data_dir, transform=None, filter_description=True
    ) -> None:
        """
        ...
        self.full_filenames = [
            os.path.join(path2images, filename) for filename in filenames
        ]
    ...
    def process_metadata(self, metadata: dict):
        ...
        processed_metadata = process_text.remove_punctuation(
            processed_metadata
        )

        processed_metadata = process_text.remove_unwant_spaces(
            processed_metadata
        )
