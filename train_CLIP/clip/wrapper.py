...
class CLIPWrapper(pl.LightningModule):
    ...
    def training_step(self, train_batch, idx):
        ...
        with torch.no_grad():
            ims = [
                F.normalize(self.model.encode_image(im), dim=1)
                for im in image_mbs
            ]
            txt = [
                F.normalize(self.model.encode_text(t), dim=1) for t in text_mbs
            ]
...
class CustomCLIPWrapper(CLIPWrapper):
        ...
        with torch.no_grad():
            ...
            loss += (
                (
                    F.kl_div(
                        F.log_softmax(
                            image_logits_notemp * self.sink_temp, dim=-1
                        ),
                        img_target,
                        reduction="batchmean",
                    )
    ...
    def encode_text(self, inputs, teacher=False):
        if self.avg_word_embs:
            sequence_output = (
                self.teacher.transformer(**inputs)[0]
                if teacher
                else self.model.transformer(**inputs)[0]
            )
            ...
            return (
                self.teacher.transformer(**inputs)[1]
                if teacher
                else self.model.transformer(**inputs)[1]
            )
    ...
    def update_teacher(self):
        for teacher, student in zip(
            self.teacher.parameters(), self.model.parameters()
        ):
