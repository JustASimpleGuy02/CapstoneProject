...
class CustomCLIPWrapper(CLIPWrapper):
    ...
    def training_step(self, train_batch, idx):
        ...
        with torch.no_grad():
            ...
            loss += (F.kl_div(F.log_softmax(image_logits_notemp * self.sink_temp, dim=-1), 
                              img_target, reduction='batchmean') 
                     + F.kl_div(F.log_softmax(image_logits_notemp.t() * self.sink_temp, dim=-1), 
                                txt_target, reduction='batchmean')) / 2 * self.kl_coeff
            self.log_dict({'loss': loss / len(ims), 
                           'acc': (acc_i + acc_t) / 2 / len(image) / len(ims)}, prog_bar=True)
        ...
        for j, mb in enumerate(image_mbs):
            ...
            loss += (F.kl_div(F.log_softmax(image_logits_notemp * self.sink_temp, dim=-1), img_target, reduction='batchmean') \
                + F.kl_div(F.log_softmax(image_logits_notemp.t() * self.sink_temp, dim=-1), txt_target, reduction='batchmean')) / 2 * self.kl_coeff
            self.manual_backward(loss)
    ...
    def encode_text(self, inputs, teacher=False):
        if self.avg_word_embs:
            sequence_output = self.teacher.transformer(**inputs)[0] \
            if teacher else self.model.transformer(**inputs)[0]
    ...
    def forward(self, images, text):
        logits = F.normalize(self.model.encode_image(images), dim=1) \
                @ F.normalize(self.encode_text(text), dim=1).t() * self.model.logit_scale.exp()
